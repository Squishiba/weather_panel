...
Modified by Jared Bin to work with the pi pico 2 without arduino.
...
void AP3216_WE::init(i2c_inst_t *i2c)
{
    i2c_ = i2c;
...
void AP3216_WE::clearInterrupt(uint8_t intType)
{
...
void AP3216_WE::setALSThresholds(float alsLowThresInLux, float alsHighThresInLux)
{
    ...
    alsLowThres = static_cast<uint16_t>(roundf(alsLowThresInLux / factor));
    alsHighThres = static_cast<uint16_t>(roundf(alsHighThresInLux / factor));
...
void AP3216_WE::setPSIntAfterNConversions(uint8_t intFilt)
{
    ...
    {
    case 1:
        intFiltVal = 0b00000000;
        break;
    case 2:
        intFiltVal = 0b00000001;
        break;
    case 4:
        intFiltVal = 0b00000010;
        break;
    case 8:
        intFiltVal = 0b00000011;
        break;
    default:
        intFiltVal = 0b00000001;
        break;
...
uint16_t AP3216_WE::getALSData()
{
    ...
    uint16_t als = (highByte << 8) + lowByte;
    return als;
}
...
{
    uint8_t out[2] = {reg, val}; // write register address, then value.
    i2c_write_blocking(i2c_, I2C_ADDR, out, 2, false);
}

uint8_t AP3216_WE::readReg(uint8_t reg)
{
    uint8_t regVal = 0; // received data
    i2c_write_blocking(i2c_, I2C_ADDR, &reg, 1, true);
    i2c_read_blocking(i2c_, I2C_ADDR, &regVal, 1, false);
    return regVal;
}
